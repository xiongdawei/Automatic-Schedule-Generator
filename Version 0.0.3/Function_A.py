...
class Function_A():
    ...
    def serach_byname(self,namee):
        session = DBSession()
        listt = []
        results = session.query(classslot).filter(classslot.name == namee).all()
        for row in results:
            subsub = []
            subsub.append(namee)
            subsub.append(row.place)
            subsub.append(row.serial)
            subsub.append(row.week)
            subsub.append(row.teacher)
            listt.append(subsub)
        return listt


    def search_day(self,namme):
        session = DBSession()
        listt = []
        for row in session.query(classslot).filter(classslot.week == namme).all():
            subsub = []
            subsub.append(row.name)
            subsub.append(row.place)
            subsub.append(row.serial)
            subsub.append(row.week)
            subsub.append(row.teacher)
    ...
    def search_all(self):
        ...
        big = []
        for i in range(1,6):
            small = a.search_day(i)
            for j in small:
                big.append(j)
        return big

    def searchh(self,namme):
        session = DBSession()
        listt = []
        for row in session.query(classslot.place.label(namme)).all():
            listt.append(row)
        return listt
...
"""
a = Function_A()
print(a.select_choice_new('cynthia'))
print(a.get_user_datacurrent('cynthia'))
